...
export const TreesBar = ({year, month}) => {
    const [data, setData] = useState([]);
    const [edge, setEdge] = useState(false); // when all 3 month's trees are 0
    const thisUserID = auth.currentUser.uid;
    
    useEffect(() => {
        const docRef = doc(db, "users", `${thisUserID}`, "Garden", `${year}`, `${month}`, "Total");
        ...
        const unsubscribe = onSnapshot(docRef, async (document) => {
            ...
            for (var i = 0; i < 2; i++) {
                const docRef = doc(db, "users", `${thisUserID}`, "Garden", `${yearVar}`, `${monthVar}`, "Total");
            ...
            <View style={{alignSelf: 'center', flex: 1, justifyContent: 'center', marginBottom: 40}}>
                ...
                >   
                    ...
                    <VictoryBar
                        ...
                        alignment="middle"
                        barWidth={25}
                        style={{ data: { fill: "#33765d" }, labels: { fontSize: 15 } }}
